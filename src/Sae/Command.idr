...
generateIpkg baseDir cfg = do
    ...
    let ipkg = configToIpkg $ record {modules = !(modulesFromSourcedir cfg.sourcedir)} cfg
...
export
runCmdWithoutConfig : Command -> IO ()
runCmdWithoutConfig Help = putStrLn helpMessage
runCmdWithoutConfig (New package) = do
    createDir package
    changeDir package
    createDir "src"
...
runCmd baseDir cfg Release = do
    runCmd baseDir cfg Build
    let executableName = if cfg.target == "javascript" then "index.js" else cfg.package
...
export
cmdFromArgs : List String -> Command
cmdFromArgs ("new"::x::_) = New x
cmdFromArgs ("build"::_) = Build
cmdFromArgs ("install"::_) = Install
cmdFromArgs ("release"::_) = Release
cmdFromArgs ("run"::_) = Run
cmdFromArgs ("fetch"::_) = FetchDeps
cmdFromArgs ("build-deps"::_) = BuildDeps
cmdFromArgs _ = Help
